...
void usage()
{
	cout << "K-Mer Counter (KMC) ver. " << KMC::CfgConsts::kmc_ver << " (" << KMC::CfgConsts::kmc_date << ")\n"
		...
		<< "  -f<a/q/m/bam/kmc/sra/srau> - input in FASTA format (-fa), FASTQ format (-fq), multi FASTA (-fm) or BAM (-fbam) or KMC(-fkmc) or SRA (-fsra) or SRA unsorted (-fsrau); default: FASTQ\n"
